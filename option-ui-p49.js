...
(function(){
  ...
  function mrsWaitForNativeOptionRows(expectedCount, done){
    var waitCount=0;
    var check=function(){
      var rowCount=mrsCountPreparedNativeRows();
  ...
  function mrsPrimeAddonSubmitCache(reason){
    ...
    _mrsAddonPrimeTimer=setTimeout(function(){
      ...
      if(_mrsAddonSubmitCache) console.info('[mrs-p49] addon submit cache primed', { reason: reason||'', addonCount: _mrsAddonSubmitCache.addonSelectedItems.length, option1: _mrsAddonSubmitCache.option1 });
    },0);
  }
  function mrsWaitForAddonSubmitCache(done, reason){
    ...
    var check=function(){
      ...
      if(ready || Date.now()>=deadline){
        console[ready?'info':'warn']('[mrs-p49] addon submit cache wait complete', {
          reason: reason||'',
          ready: ready,
  ...
  function mrsBuildNoteBasketParams(optionValue){
    var productName=mrsGetProductName();
    return {
  ...
  function mrsSubmitWithAddon(type, restoreFns, _origCheck){
    ...
    if(!cache||!cache.mainSelectedItem){
      console.warn('[mrs-p49] addon submit fallback: missing selected_item');
      mrsFinishSubmitFlow(restoreFns, _origCheck);
      restoreFns.alert('선택한 옵션 정보를 다시 불러오지 못했습니다. 다시 시도해주세요.');
      return;
    }
    console.info('[mrs-p49] addon manual submit', { type: type, mainSelectedItem: cache.mainSelectedItem, addonCount: cache.addonSelectedItems.length, warmed: !!_mrsAddonSubmitCache });
    fetch('/exec/front/order/basket/',{
      ...
      console.info('[mrs-p49] addon manual submit result', { status: payload.status, result: result });
      ...
      console.warn('[mrs-p49] addon manual submit failed', err);
      mrsFinishSubmitFlow(restoreFns, _origCheck);
  ...
  function mrsEnsureDirectSubmitReady(type, optionValues, done, reason){
    ...
    var proceedAfterNative=function(nativeReady){
      ...
      mrsSyncPenAddonSelection(_penAdded,function(syncReady){
        if(!syncReady) console.warn('[mrs-p49] proceeding after pen addon sync timeout', { enable: _penAdded, type: type, reason: tag });
        if(!(type===1&&_penAdded)){
        ...
        mrsWaitForAddonSubmitCache(function(cacheReady){
          if(!cacheReady) console.warn('[mrs-p49] proceeding after addon cache wait timeout', { type: type, reason: tag });
    ...
    mrsWaitForNativeSelection(optionValues,function(nativeReady){
      ...
      console.warn('[mrs-p49] selection-time prep not ready by submit, running fallback sync', {
        type: type,
        reason: tag,
        ...
        if(cls.indexOf('kakao')!==-1||cls.indexOf('kakaopay')!==-1||cls.indexOf('naverpay')!==-1||cls.indexOf('naver-pay')!==-1||cls.indexOf('npay')!==-1||cls.indexOf('checkout_btn')!==-1||cls.indexOf('Npay')!==-1){
          ...
          var fastClick=function(){
            ...
            mrsEnsureDirectSubmitReady(1, optionValues, function(state){
              if(!state.syncReady) console.warn('[mrs-p49] pay click continuing after readiness wait timeout', state);
              mrsMakeAddonOptional();
              _mrsPayBypass=true;
              clickTarget.click();
            },'pay-bypass');
